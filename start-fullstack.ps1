...
try {
    ...
    Write-Host "⚠️  Could not verify API startup" -ForegroundColor Yellow
}

if ($apiHealth) {
    Write-Host "✅ Backend API started successfully" -ForegroundColor Green
} else {
    Write-Host "⚠️  Backend API may not be responding" -ForegroundColor Yellow
}
...
Write-Host ""
Write-Host "⚙️  Both services are running in separate windows." -ForegroundColor Gray

...
Detailed architecture, flow, and file-structure diagrams are available in [`docs/ARCHITECTURE_AND_FLOWS.md`](docs/ARCHITECTURE_AND_FLOWS.md).

```mermaid
flowchart LR
  V[Video Input] --> I[Ingestion Pipeline]
  I --> X[Multimodal Indexing]
  X --> R[Hybrid RAG]
  R --> A[Multi-Agent Reasoning]
  A --> O[Forensic Report]
```

...
import { Upload, FileVideo, HardDrive, TerminalSquare, Activity, ChevronRight } from "lucide-react";
...
export default function Home() {
  ...
  const [processingMode, setProcessingMode] = useState<"fast" | "balanced" | "full">("balanced");
      ...
      <div className={styles.grid}>
        ...
        <div className={styles.card}>
          ...
          <select
            ...
            onChange={(e) => setProcessingMode(e.target.value as "fast" | "balanced" | "full")}
            disabled={uploading}
            className={styles.select}
          >
            <option value="fast">Fast (No Speaker Diarization / No Visual Emotion)</option>
            <option value="balanced">Balanced (Speaker Auth + Transcript Emotions)</option>
            <option value="full">Full Forensics (Deep Visual Emotion + Speaker + Transcript)</option>
          </select>
          ...
          >
            ...
            <p className={styles.uploadText}>Drop video file here or click to browse</p>
            <button className={styles.button} disabled={uploading} type="button">
              Select Video
            </button>
            <input
              ref={fileInputRef}
                ...
                {existingVideos.map((v) => (
                    ...
                    <div>
                        ...
                        <div className={styles.videoMeta}>
                            <FileVideo size={14} /> ID: {v.video_id}
                        </div>
                    </div>
                    {v.segments > 0 && <span className={styles.badge}>{v.segments} segments</span>}
                    </div>
                ))}
                </div>
            ) : (
            ...
            <div className={styles.card} style={{ flex: 1 }}>
                ...
                <div className={styles.grid}>
                    ...
                    {selectedVideo ? (
                        ...
                        <span className={`${styles.buttonSecondary} ${styles.disabled}`}>
                            <Activity size={16} /> Review Timeline
                        </span>
                    )}
                </div>
